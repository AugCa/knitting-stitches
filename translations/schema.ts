/**
 * Translation schema for knitting-operations.
 *
 * Each language file at `translations/<iso-code>.json` follows the
 * `LanguageFile` shape below. Operations are keyed by their canonical id
 ...
 * base operation says (typically English).
 *
 * Translations are deliberately separate from `operations/*.json` so
 * native speakers can iterate on their own language file without
 * touching the canonical operation definitions, and so consumers can
 * choose at runtime which language(s) to merge in.
 */

export interface OperationTranslation {
...
export interface LanguageFile {
  ...
  maintainers?: string[];
  /** Translations keyed by operation id. */
  operations: Record<string, OperationTranslation>;
}
