...
- A French knitter contribute French canonical names and abbreviations without touching the canonical operation definitions
...
|------|----------|--------|
| `en.json` | English | **Fully populated** (extracted from `operations/*.json`) |
| `de.json` | German (Deutsch) | Skeleton — every operation_id is present with empty fields |
...
**All four fields per operation are optional.** Leave any out and the loader falls back to the base operation's English value. So you can populate just the canonical name and skip the rest if you're not sure.

## How to contribute a translation

1. Open the file for your language (e.g. `de.json` for German).
2. Find an operation by id (the keys are the same across all language files — they match `operations/*.json`).
...
You don't need to translate every operation. Even a partial PR that fills in the basics (knit, purl, k2tog, ssk, yo) is valuable — the loader handles partial coverage gracefully.

## Adding a new language

1. Create `translations/<iso-code>.json` modeled on `de.json` (skeleton structure with every operation_id present).
...
- `mergeTranslations(base: KnittingOperation, translation?: OperationTranslation): KnittingOperation`
