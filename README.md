...
A machine-readable semantic vocabulary of knitting stitches.
...
- **Stitches** (`stitches/*.json`) — ~250 hand-curated knitting stitches: knits, purls, decreases, increases, cables, brioche, slipped stitches, edges, cast-ons, bind-offs, structural markers. Each stitch carries:
...
- **Registry** (`registry.ts`) — `StitchRegistry` with lookup-by-id, abbreviation, tag, category
...
```bash
pnpm add @knitr/knitting-stitches
```
...
**Coverage over efficiency.** When in doubt, split. Don't merge stitches that real publishers treat as distinct, even if their chart symbols look identical (`cdd` vs `cdd_tbl` vs `s2kp`, for example).
